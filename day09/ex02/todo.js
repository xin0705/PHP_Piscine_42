...
function add_todo(todo, todo_id)
{
	...
	new_todo_div.setAttribute("onclick", "rm_todo(this)");
	setCookie(cookie_prefix + new_todo_div.id, todo);
}

function add_item()
{
	var new_todo = prompt("Please write your next TODO task");
...
function rm_todo(todo)
{
	if (confirm("Will you confirm to remove the item?"))
	{	
		todo.remove();
